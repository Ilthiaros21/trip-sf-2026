...
## Estado
App con estética ref1 (pastel, Familjen/Hanken, liquid glass en nav) + spine continuo por categoría con reveal de scroll (línea de lectura + cercanía al fondo), sin la línea atravesando los dots + tarjetas 3D con progreso tri-estado (pulso/confetti, fill realmente animado) + feedback de tap perceptible y sincronizado (tarjeta+hora+tipo), sin quedarse atorado + tipo de tarjeta junto a la hora + hotel Aloft Sunnyvale + Itinerario granular sin resumen de día + Info práctica de 4 tabs, en producción vía GitHub Pages. Docs sincronizados.
