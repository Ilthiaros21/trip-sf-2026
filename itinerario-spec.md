...
## Decisiones — Feedback de tap más lento y perceptible, fill de progreso arreglado

- **Amplitud de press aumentada**: `.card-head:active` estaba en `scale(0.99)` (1% — invisible), `.check-row`/`.card` (press del :has) en `scale(0.995)` (0.5% — también invisible). Subido a `0.97` en tarjetas/filas, mantenido/ajustado proporcional en botones que ya tenían más amplitud.
- **Press asimétrico**: bajar el dedo (`:active`) sigue rápido (`--dur-instant`, 110ms) para que se sienta responsivo, pero soltar ahora es más lento (`--dur-settle`, nuevo token, 260ms) — el ojo necesita ver el regreso para registrar que hubo movimiento. Técnica: la regla `:active` redeclara su propia `transition` más rápida; la regla base (que gobierna la vuelta al quitar `:active`) se queda con la más lenta. Aplicado en tarjetas, botón de progreso, checklist, links de contacto, tabs, mode-switch.
- **El fill del progreso (dot y botón) no se animaba — nunca, es un bug real, no de velocidad**: `background: transparent → conic-gradient(...)` es un cambio de TIPO de valor, no interpolable por CSS — el navegador lo cambia de golpe a media transición en vez de animarlo, sin importar qué duración se declare. Arreglado con `@property --progress-frac` (custom property tipada, transicionable) — ahora el conic-gradient es siempre la misma función, solo se mueve el corte (0%/50%/100%), así que sí anima de verdad. El check de "hecho" también dejó de aparecer con `display:none→block` (tampoco animable) — ahora hace fundido + pop encima del aro ya lleno.

## Decisiones — Dots sin la línea atravesándolos, tarjeta ya no se atora, glow sincronizado

- **Los aros del spine dejaban ver la línea a través de su centro hueco**: el estado "vacío" del conic-gradient de `.block-dot` usaba `transparent`, y el dot pinta encima del spine (z-index más alto) — un aro con centro transparente no tapa lo que hay detrás, así que la línea del spine se veía pasar justo por en medio de cada aro sin tocar, dando la sensación de línea rota/pareja no uniforme. Arreglado: el "vacío" ahora es `var(--bg)` (el fondo de la página, que es lo que de verdad hay detrás del dot) en vez de `transparent` — el mismo conic-gradient sigue animando igual, pero ahora tapa limpio en vez de dejar ver la línea.
- **La tarjeta se quedaba "atorada" en su look de presionada — bug real de iOS Safari, no de ajuste**: el feedback de press vivía en `:active` sobre `.card-head`, que es un `<div role="button">` (no puede ser `<button>` real porque adentro vive un `<button class="progress-btn">` de verdad, y HTML no permite botón dentro de botón). iOS Safari tiene un bug conocido: `:active` en elementos no nativos puede no limpiarse solo después de soltar el toque. Reemplazado por completo con una clase `.is-pressed` puesta/quitada a mano en JS vía `pointerdown`/`pointerup`/`pointercancel`, con `setPointerCapture` en el `pointerdown` para que soltar el dedo en cualquier parte de la pantalla siga limpiando el estado correctamente. Esto también permitió borrar el viejo `:has(.card-head:active)` + `@supports selector(:has(a))` — ya no hace falta, `.is-pressed` funciona igual en todos los navegadores sin rama de compatibilidad.
- **Glow de la tarjeta, la hora y el tipo ahora sincronizados**: antes `.block-time` y `.cat-label` brillaban con una sola duración fija mientras que la tarjeta ya tenía bajada rápida/subida lenta separadas — se veía el glow apagarse mientras la tarjeta todavía estaba regresando. Ahora los tres comparten el mismo disparador (`.card.is-pressed`) y el mismo par de tiempos (bajada `--dur-instant`, vuelta `--dur-settle`) — se sienten como un solo gesto. Intensidad bajada un poco también (glow "leve", como se pidió).

## Decisiones — Track del spine visible de verdad, entrada de tarjetas al cambiar de día

- **El track (parte sin llenar del spine) se veía mucho más delgado que el relleno**: medido con `getBoundingClientRect()`, track y relleno miden exactamente los mismos 7px de ancho y el mismo centro — no era un problema de layout. El track usaba `var(--border)` al 16% de opacidad, casi invisible contra el fondo pastel, mientras el relleno es un color 100% opaco — al ojo, eso se lee como dos anchos distintos aunque midan igual, dando la sensación de "no cubre toda la línea". Arreglado subiendo la opacidad del track (`color-mix` con `--text` al 22%) y bajando el blur del relleno de 2px a 1px (en una tira de 7px, 2px de blur le comía visualmente el borde).
- **Las tarjetas no animaban al cambiar de día si ya estaban visibles en pantalla — bug real de arquitectura, no cosmético**: `animation-timeline: view()` (el mecanismo scroll-driven) apagaba por completo el stagger de entrada normal (`riseIn`, basado en tiempo) en cualquier navegador que lo soporta. El problema: `animation-timeline: view()` solo reacciona a SCROLL — cambiar de pestaña no scrollea nada, así que para cualquier tarjeta ya visible en pantalla en ese momento (la mayoría, típicamente), su "entrada" a la vista ya había "pasado" conceptualmente, y el navegador la resuelve directo en su estado final (opacity:1) sin transición visible. Arreglado haciendo que las dos animaciones convivan en vez de que una apague a la otra: `riseIn` (reloj normal, `animation-timeline: auto`) siempre dispara al activar el día, y `flapReveal` (scroll-driven) se suma encima como plus para tarjetas que entran a la vista por primera vez más abajo. Sintaxis: `animation-timeline: auto, view();` con dos animaciones separadas por coma en una sola regla — escrito solo dentro del `@supports` que ya garantiza que el navegador entiende `view()`, así que siempre parsea bien.

## Estado
App con estética ref1 (pastel, Familjen/Hanken, liquid glass en nav) + spine continuo por categoría con reveal de scroll (línea de lectura + cercanía al fondo), track visible y sin la línea atravesando los dots + tarjetas 3D con progreso tri-estado (pulso/confetti, fill realmente animado) + feedback de tap perceptible y sincronizado (tarjeta+hora+tipo), sin quedarse atorado + entrada garantizada al cambiar de día (no solo por scroll) + tipo de tarjeta junto a la hora + hotel Aloft Sunnyvale + Itinerario granular sin resumen de día + Info práctica de 4 tabs, en producción vía GitHub Pages. Docs sincronizados.
